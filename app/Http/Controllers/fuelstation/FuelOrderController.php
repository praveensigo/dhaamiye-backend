    ...
    public function add(Request $request)
    {
        ...
        if ($validator->fails()) {
           ...
           if ($fields['coupon_code']) {
            ...
            $order->fuel_station_id = $user_id;
            $order->order_type = $fields['order_type'];
            $a= $order->fuel_quantity_price = $fuel_quantity_price;
            $delivery =$this->delivery_charge($latitude, $longitude,$user_id);
...
public function assignDriver(Request $request)
{
    ...
    if ($validator->fails()) {
    ...
    } else { $driver = DB::table('drivers')->select('drivers.*')
        ->where('id', $fields['driver_id']) ->where('fuel_station_id',$user_id)
        ->first();
       ...
       if($driver){
        ...
        if ($result) {
            ...
            $res = Response::send('false',
                [],
                $message = __('error.driver_assign_failed'),
                $code = 400);
        }
    }
...
public function index(Request $request)
{
    ...
    if ($validator->fails()) {$errors = collect($validator->errors());
        $res = Response::send('false', $data = [], $message = $errors, $code = 422);
         ...
         'p1.name_en as customer_name_en', 'p1.name_so as customer_name_so', 'p2.name_en as driver_name_en', 'p2.name_so as driver_name_so', 'p1.mobile as customer_mobile','p2.mobile as driver_mobile' ,'p1.country_code_id','customer_orders.status', 'customer_orders.total', 'customer_orders.created_at','customer_orders.added_by as customer_order_added_by','customer_orders.added_user as customer_order_added_user')
