...
use App\Models\fuelstation\CustomerOrder;
use App\Models\fuelstation\CustomerOrderPayment;
use App\Models\fuelstation\Driver;
use App\Models\fuelstation\DriverPayment;
...
class DriverController extends Controller
{
    ...
    public function update(Request $request)
    {
        ...
        $validator = Validator::make($request->all(), [
            ...
            'truck' => 'required|numeric|exists:trucks,id',
            'image' => 'nullable|max:4096|mimes:png,jpg,jpeg,pdf',
            'passport' => 'nullable|max:4096|mimes:png,jpg,jpeg,pdf',
            'license' => 'nullable|max:4096|mimes:png,jpg,jpeg,pdf',
            'license_expiry' => 'required|date_format:Y-m-d|after:today',
...
/*UPDATE STATUS*/
    ...
    {$auth_user = Auth::user();
        ...
        if ($validator->fails()) {
            ...
            $driver = Driver::where('id',$fields['id'])->where('drivers.fuel_station_id', $user_id)->first();
          if($driver) { $driver->status = $fields['status'];
            $result = $driver->save();
        ...
        if ($validator->fails()) {
            ...
            if ($fields['status'] == '0') {
                ...
                foreach ($payments as $payment) {
                    ...
                    $payment->total_completed =DriverPayment::query()
                        ->select(
                            DB::raw('SUM(amount) AS total_completed_amount')
                        )
                        ->whereDATE('created_at', $payment->day)
                    ...
                    $payment->mobile_completed =DriverPayment::query()
                        ->select(
                        ...
                        ->whereDATE('created_at', $payment->day)
                        ->first()->mobile_amount;
                    $payment->cash_completed =DriverPayment::query()
                        ->select(
